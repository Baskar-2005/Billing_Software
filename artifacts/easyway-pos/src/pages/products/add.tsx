...
export default function AddProduct() {
  ...
  return (
    <AppLayout hideNav>
      <TopBar title="Add Product" showBack onBack={() => setLocation("/products")} />
      ...
      <div className="p-4 max-w-md mx-auto w-full">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 bg-card p-5 rounded-2xl border border-border shadow-sm">
            ...
            <Button type="submit" className="w-full h-12 rounded-xl text-lg font-semibold mt-4" disabled={createProduct.isPending}>
              {createProduct.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : "Save Product"}
            </Button>
          </form>
        </Form>
      </div>
