      ...
      { data: { pin: pinValue } },
      {
        onSuccess: (res) => {
          if (res.authenticated) {
            // Instantly update cache so AuthGuard sees authenticated=true
          ...
          {[...Array(6)].map((_, i) => (
            <motion.div
              ...
              className={cn(
                ...
                pin.length > i ? "bg-primary scale-100" : "bg-border scale-75"
              )}
              animate={pin.length === i ? { scale: [1, 1.2, 1] } : {}}
              transition={{
                repeat: pin.length === i ? Infinity : 0,
                duration: 1.5,
              }}
            />
          ))}
        ...
        <div className="w-full max-w-[280px] grid grid-cols-3 gap-x-6 gap-y-6">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((digit) => (
            <motion.button
              ...
              className="w-20 h-20 rounded-full flex items-center justify-center text-3xl font-semibold text-foreground bg-card shadow-sm border border-border mx-auto disabled:opacity-50"
            >
              {digit}
            </motion.button>
          ))}
          ...
          <motion.button
            ...
            disabled={pin.length === 0 || login.isPending}
